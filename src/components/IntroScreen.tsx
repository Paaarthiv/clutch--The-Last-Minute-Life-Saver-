...
interface RepelNode { ox: number; oy: number; rx: number; ry: number; }

function ParticleSphere({ nodes = [], onGeo }: { nodes?: RepelNode[]; onGeo?: (g: { cx: number; cy: number; r: number }) => void }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      cx = W / 2; cy = H / 2; R = Math.min(W * 0.46, H * 0.62);
    ...
    const render = () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        for (let k = 0; k < nds.length; k++) {
          const nd = nds[k];
          const nxp = cx + nd.ox * R, nyp = cy + nd.oy * R;
          if (Math.abs(sx - nxp) < nd.rx + 6 && Math.abs(sy - nyp) < nd.ry + 6) { hidden = true; break; }
        }
        if (hidden) continue;
        pts.push({ sx, sy, z });
      }
